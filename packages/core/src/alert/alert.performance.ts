...
describe('cds-badge performance', () => {
  ...
  it(`should bundle and treeshake alert`, async () => {
    expect((await testBundleSize('@cds/core/alert/register.js')).kb).toBeLessThan(24);
  });

  it(`should render 1 alert under 15ms`, async () => {
    expect((await testRenderTime(alert)).duration).toBeLessThan(20);
  });

  it(`should render 10 alert under 25ms`, async () => {
    expect((await testRenderTime(alert, { iterations: 10 })).duration).toBeLessThan(25);
  });

  it(`should render 1 alert group with 1 alerts under 15ms`, async () => {
    expect((await testRenderTime(alertGroup(1))).duration).toBeLessThan(20);
  });

  it(`should render 1 alert group with 10 alerts under 15ms`, async () => {
